...
def getters():
    '''-----get-----'''
    descripcion = app.desc.get()
    periodos = int(app.period.get())
    esperados = int(app.period.get())
    cantidad_min = int(app.mcp.get())
    cantidad_max = int(app.maxcp.get())
    seleccion = int(app.tpp.get())
    marcado = int(app.tpm.get())
    pago = int(app.tppago.get())

    dist = app.distribucion.get().split(",")
    distribution_arr = []
    for i in dist:
        distribution_arr.append(int(i))
    sum = 0
    for i in distribution_arr:
        sum += i
    cajas_arr = []
    box = app.cajas.get().split(",")
    for i in box:
        cajas_arr.append(int(i))
    '''if(conditions):
            abrir denuevo la app indicando errores ingresados'''

    # if ningun warning, window.destroy()
    app.window.destroy()
