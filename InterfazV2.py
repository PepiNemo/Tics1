...
class Simulador:
    ...
    def simulate(self):
        ...
        try:
            self.Desc = self.desc.get()
            BoolDesc = True
        finally:
            self.Desc = self.desc.get()
            if len(self.Desc) <= 0 or len(self.Desc) > 60:
                BoolDesc = False
                print("Error en la descripcion")

        try:
            self.Horas = int(self.horas.get())
            BoolHoras = True
        except:
        ...
        try:
            self.Clientes = int(self.clientes.get())
            BoolClientes = True
        except:
        ...
        if BoolName and BoolDesc and BoolHoras and BoolClientes and BoolMinProd and BoolMaxProd and BoolTsel and BoolTmar and BoolTpago and BoolDist and BoolCajas:
            ...
            if not BoolMaxProd:
                lblMax['text']="Error en cantidad maxima de productos, debe ser mayor que "+str(self.Min)+"."
...
def retornar():
    ...
    #print("numero de clientes 2:" ,str(Clientes))
    root.mainloop()
    print("Enviando Datos")
    print(app.Nombre)
    lista = [app.Nombre, app.Desc,app.Horas,app.Clientes, app.Cajas,app.Distrib, app.Min,app.Max, app.ts, app.tm,app.tp]
    return lista
